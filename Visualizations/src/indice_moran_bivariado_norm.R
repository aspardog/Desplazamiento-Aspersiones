...
merge_data.sf <- colombia.sf %>%
  ...
  select(codmpio, lag1_ruv_desplazamiento_forzado_pop, spraying_norm, dpto_ccdgo) %>%
...
x <- merge_data.sf$desplazamiento_forzado_pop
y <- merge_data.sf$aspersiones_norm
