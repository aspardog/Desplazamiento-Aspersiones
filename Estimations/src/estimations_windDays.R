...
firstStage <- plm(data = merge_data.df,
                  formula = paste0("spraying_norm ~  windIVDaysRMBOS +", paste(controles_fe_pop, collapse = "+")),
...
restExcl1 <- plm(data = antimerge_data.df,
                 formula = paste0("lag1_ruv_desplazamiento_forzado_pop ~  windIVDaysRMBOS +", paste(controles_fe_pop, collapse = "+")),
...
stargazer(
  ...
  dep.var.caption = "Variable dependiente: Desplazamiento Forzado",
  keep = "spraying_norm",
  se = list( IVFE[, 2], IVFE2[, 2], IVFE3[, 2]), 
...
ggsave(dinamicPlot, filename = "Visualizations/output/DinamicEffectsWindIV.png", dpi = 320, width = 10, height = 7.5)
...
controles_fe_3month <- c('night_lights', 
                         ...
                         'sum_minas_pop', 'sum_reclutamiento_pop', 'sum_homicidio', 'sum_desaparicion_pop')
...
IV3Month <- plm(data= merge_data.df,
                formula = 
                  paste0("sum_desplazamiento_pop ~ spraying_norm | windIVDaysRMBOS"),
                effect = "twoways",
                model = "within",
                index=c("codmpio", "date"))
summary(IV3Month)
